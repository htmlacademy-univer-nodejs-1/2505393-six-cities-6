...
} from './categories-data/categories-data';
import {
  setTickets,
  setTicketsNew,
  setTicketsDiscussed,
  setTicketsLoading,
} from './tickets-data/tickets-data';
...
>('data/fetchCategories', async (_arg, { dispatch, extra: api }) => {
  try {
...
>('data/fetchTickets', async (_arg, { dispatch, extra: api }) => {
  try {
...
>('data/fetchTicketsNew', async (_arg, { dispatch, extra: api }) => {
  try {
...
>('data/fetchTicketsDiscussed', async (_arg, { dispatch, extra: api }) => {
  try {
...
>('data/fetchComments', async (ticketId, { dispatch, extra: api }) => {
  try {
...
>('user/registrationUser', async (userData, { dispatch, extra: api }) => {
  try {
    const postData = await api.post<CreateUserWithIdDto>(
      APIRoute.Registration,
      adaptSignupToServer(userData)
    );

    if (postData.status === HTTP_CODE.CREATED) {
      const postAvatarApiRoute = `/${postData.data.id}/avatar`;
...
>('user/loginUser', async (userData, { dispatch, extra: api }) => {
  try {
...
>('user/logoutUser', async (_arg, { dispatch, extra: api }) => {
  // If there is a logout endpoint, use it; otherwise, just clear the token.
  if (APIRoute.Logout && APIRoute.Logout.length > 0) {
    try {
      await api.delete(APIRoute.Logout);
    } catch (error) {
      // Ignore errors from logout request? Or handle?
      errorHandle(error);
    }
  }
  dropToken();
...
>('data/editTicket', async (ticketData, { dispatch, extra: api }) => {
  try {
    const postData = await api.patch<UpdateOfferDto>(
      `${APIRoute.Tickets}/${ticketData.id}`,
      adaptEditTicketToServer(ticketData)
    );
...
>('data/createTicket', async (ticketData, { dispatch, extra: api }) => {
  try {
    const postData = await api.post<OfferDto>(
      APIRoute.Tickets,
      adaptCreateTicketToServer(ticketData)
    );
...
>('data/removeTicket', async (ticketId, { dispatch, extra: api }) => {
  try {
...
>('data/editTicket', async (commentData, { dispatch, extra: api }) => {
  try {
    await api.post<CreateOfferDto>(
      APIRoute.Comments,
      adaptCreateCommentToServer(commentData)
    );
